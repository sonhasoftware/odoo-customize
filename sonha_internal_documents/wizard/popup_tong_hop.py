...
class PopupTongHop(models.TransientModel):
    _name = 'popup.tong.hop'
    ...
    def action_confirm(self):
        ...
        # 1. Xóa dữ liệu cũ
        self.env['bao.cao.tong.hop'].search([]).unlink()

        # 2. Gọi hàm SQL
        self._cr.execute("""
                SELECT * FROM fn_bao_cao_duyet_van_ban_th(%s, %s)
            """, (date_from, date_to))
        rows = self._cr.dictfetchall()

        model = self.env['bao.cao.tong.hop']
