...
class WizardExistCar(models.TransientModel):
    ...
    driver_rent_car = fields.Many2one('hr.employee', "Lái xe", domain="[('company_id', '=', company_rent_car)]")
    ...
    def action_confirm(self):
        ...
        if (self.parent_id.driver and self.parent_id.driver.id != self.driver.id) or (
                ...
                self.parent_id.driver_rent_car and self.parent_id.driver_rent_car.id != self.driver_rent_car.id):
            edit_infor = 1
        ...
        self.parent_id.write({
            ...
            'driver_rent_car': self.driver_rent_car.id if self.type == 'rent_car' else "",
