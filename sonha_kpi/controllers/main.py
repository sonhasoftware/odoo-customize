...
class DataChart(http.Controller):
    ...
    def export_kpi_month_excel(self, department_id=None, year=None, month=None, **kwargs):
        ...
        worksheet.merge_range('I2:I3', 'CHỈ TIÊU ĐO LƯỜNG', wrap_format)
        worksheet.merge_range('J2:L2', 'Σ ĐIỂM ĐƠN VỊ\nĐÁNH GIÁ', wrap_format)
        worksheet.write('J3', 'Kết quả hoàn thành', wrap_format)
        worksheet.write('K3', 'Điểm đạt', wrap_format)
        worksheet.write('L3', 'Điểm quy đổi theo tỷ trọng', wrap_format)
        worksheet.merge_range('M2:O2', 'Σ ĐIỂM CẤP THẨM\nQUYỀN ĐÁNH GIÁ', wrap_format)
        worksheet.write('M3', 'Kết quả hoàn thành', wrap_format)
        worksheet.write('N3', 'Điểm đạt', wrap_format)
        worksheet.write('O3', 'Điểm quy\nđổi theo\ntỷ trọng', wrap_format)
        worksheet.merge_range('P2:P3', 'NHẬN XÉT\nCỦA CẤP CÓ\nTHẨM QUYỀN', wrap_format)
        ...
        for rec in records:
            ...
            worksheet.merge_range(row, 8, row + 1, 8, rec.dv_do_luong or '', text_format)
            worksheet.merge_range(row, 15, row + 1, 15, rec.note or '', text_format)

            worksheet.write(row, 4, "Khối lượng \ncông việc \nthực hiện", cell_format)
            worksheet.write(row, 6, rec.diem_chuan_amount_work or 0.0, cell_format)
            worksheet.write(row, 9, f"{rec.kq_hoan_thanh_amount_work * 100}%", cell_format)
            worksheet.write(row, 10, rec.diem_dat_dv_amount_work or 0.0, cell_format)
            worksheet.write(row, 11, rec.quy_doi_dv_amount_work or 0.0, cell_format)
            worksheet.write(row, 12, None, cell_format)
            worksheet.write(row, 13, rec.diem_dat_tq_amount_work or 0.0, cell_format)
            worksheet.write(row, 14, rec.quy_doi_tq_amount_work or 0.0, cell_format)

            worksheet.write(row + 1, 4, "Chất lượng \ncông việc \nthực hiện", cell_format)
            worksheet.write(row + 1, 6, rec.diem_chuan_matter_work or 0.0, cell_format)
            worksheet.write(row + 1, 9, f"{rec.kq_hoan_thanh_matter_work * 100}%", cell_format)
            worksheet.write(row + 1, 10, rec.diem_dat_dv_matter_work or 0.0, cell_format)
            worksheet.write(row + 1, 11, rec.quy_doi_dv_matter_work or 0.0, cell_format)
            worksheet.write(row + 1, 12, None, cell_format)
            worksheet.write(row + 1, 13, rec.diem_dat_tq_matter_work or 0.0, cell_format)
            worksheet.write(row + 1, 14, rec.quy_doi_tq_matter_work or 0.0, cell_format)
