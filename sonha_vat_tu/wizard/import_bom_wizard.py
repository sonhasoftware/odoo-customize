...
class ImportBomWizard(models.TransientModel):
    ...
    def action_import(self):
        ...
        for row_idx, row in enumerate(rows[1:], start=2):
            ...
            key = (self.company_id.id, ma_tp, ma_nvl)
            if key in seen:
                errors.append(_('Dòng %d: trùng bộ (Đơn vị, Mã TP, Mã NVL) trong file.') % row_idx)
                continue
            seen.add(key)

            sl_dinh_muc = self._to_float_or_error(row[4], 'Số lượng định mức / 1 sản phẩm', row_idx, errors)
            do_day = self._to_float_or_error(row[5], 'Độ dày', row_idx, errors)
            kho_1 = self._to_float_or_error(row[6], 'Khổ 1', row_idx, errors)
            kho_2 = self._to_float_or_error(row[7], 'Khổ 2', row_idx, errors)
        ...
        if vals_list:
            ...
            existing_keys = {(r.company_id.id, r.ma_tp, r.ma_nvl) for r in existing}
            for vals in vals_list:
                if (vals['company_id'], vals['ma_tp'], vals['ma_nvl']) in existing_keys:
                    errors.append(_('Đã tồn tại BOM cho Đơn vị=%s, Mã TP=%s, Mã NVL=%s.')
                                  % (self.company_id.display_name, vals['ma_tp'], vals['ma_nvl']))
